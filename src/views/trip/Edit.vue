...
import { useTripEdit } from './tripmixin';

const route = useRoute();
const tripId = route.params.id;

const { loading, saving, errorMessage, deleteDialog, submitted, validationErrors, tripData, vehicles, staffList, customerList, router, fetchTripData, fetchVehicles, fetchStaffList, fetchCustomers, deleteTrip, handleSubmit } = useTripEdit(tripId);
...
onMounted(async () => {
    ...
    await Promise.all([fetchTripData(), fetchVehicles(), fetchStaffList(), fetchCustomers()]);
});
